...
if __name__ == '__main__':
    ...
    df_train = pd.read_csv('../../training_caip_contest.csv', header=None)
    # df_train = pd.concat([df_train] * 8)
    x_train = np.load('data/features_pred_resnext_aug_aar_3.npy')
    y_train = np.array(df_train.iloc[:, 1])
    # Shuffle indices
    idx = np.random.shuffle(np.arange(x_train.shape[0]))
    ...
    x_test = np.load('data/features_pred_resnext_aug_aar_test.npy')
    ...
    out_train = rfr.predict(x_train)
    dump(rfc, f'data/tlrf_rfc_aug_{n_estimators}_{min_samples_leaf}_{max_features}_{random_state}.joblib') 
    dump(rfr, f'data/tlrf_rfr_aug_{n_estimators}_{min_samples_leaf}_{max_features}_{random_state}.joblib') 
    ...
    AAR, MAE, *_, sigmas, maes = aar(y_train, out_train)
    print(f'ResNext: AAR on training: {AAR}')
    maes_str = '\t'.join([f'{m:.03f}' for m in maes])
    sigmas_str = '\t'.join([f'{np.sqrt(m):.03f}' for m in sigmas])
