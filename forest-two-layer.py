...
if __name__ == '__main__':
    ...
    dump(rfc, f'./data/tlrf_resnext_rfc_{n_estimators}_{min_samples_leaf}_{max_features}_{random_state}.joblib') 
    dump(rfr, f'./data/tlrf_resnext_rfr_{n_estimators}_{min_samples_leaf}_{max_features}_{random_state}.joblib') 
    ...
    print(f'ResNext: MAE on valdiation: {ae.mean()} +/- {ae.std()}')
    ...
    rrfc.fit(x_train_origin, y_train_group)
    print(f'Time it took for refinement of rfc: {time.time() - t0:.3f} ms.')
    outc = rrfc.predict_proba(x_test).argmax(axis=1)
    y_pred_group = outc.argmax(axis=1)
    conf_mat = confusion_matrix(y_test_group, y_pred_group, normalize='true')
    ...
    sns.heatmap(conf_mat, annot=True, fmt='.2f', cbar=False,
                xticklabels=["< 10", "10-19", "20-29", "30-39", "40-24", "50-59", "60-69", "> 70"],
                yticklabels=["< 10", "10-19", "20-29", "30-39", "40-24", "50-59", "60-69", "> 70"])
    plt.savefig('heaptmap_refined.png')
    for i in range(8):
        y_pred_i = y_pred_group == i
        y_true_i = y_test_group == i
        print(f"Accuracy for group {i}: {accuracy_score(y_true_i, y_pred_i)}")
    ...
    #refine rfr
    t0 = time.time()
    rrf = RefinedRandomForest(rfr, C = 0.01, n_prunings = 1)
